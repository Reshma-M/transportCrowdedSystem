...
const long NUMBER_OF_UPDATES_TO_SETTLE = 10000000;
const long NUMBER_OF_UPDATES = NUMBER_OF_UPDATES_TO_SETTLE * 10;
int NUMBER_OF_SNAPSHOTS = 1500;
const int NUMBER_OF_VESICLES = 1000;
const int NUMBER_OF_SITES = 8000;
...
const int DENSITY_AVERAGED_OVER = 1000;
const int NUMBER_OF_DENSITY_SNAPSHOTS = 1250;
const double DENSITY_SNAPSHOT_INTERVAL = 0.2;
...
public:
    ...
    void setIsBleached(bool b)
    {
        isBleached = b;
    }

    Vesicle(long vId, int mId, int sId, VesicleType vesicleType, long cty, long ctr, bool b)
    {
        setVesicleId(vId);
...
{
    Vesicle vesicle = Vesicle(0, 0, 0, SMOOTH_ANTEROGRADE, 0, 0, false);
...
void makeVesicles()
{
    ...
    while (numberOfVesicles > 0)
    {
        ...
        {
            VESICLES.insert(Vesicle(generateVesicleId(), microtubuleNumber, siteNumber, vT, 0, 0, false));
            numberOfVesicles--;
        }
    }
}
...
void makeBreak(int microtubuleNumber, int startSite)
{
    ...
    {
        VESICLES.insert(Vesicle(generateVesicleId(), microtubuleNumber, (startSite + i) % NUMBER_OF_SITES, STANDSTILL, 0, 0, false));
    }
}
...
void changeTrackVesicle(Vesicle v, NeighbourMicrotubule neighbourMicrotubule)
{
    Vesicle newVesicle = Vesicle(v.getVesicleId(), (v.getMicrotubuleId() + NUMBER_OF_MICROTUBULES + (1 * neighbourMicrotubule)) % NUMBER_OF_MICROTUBULES,
                                 v.getSiteId(), v.getType(), v.getNumberOfChangetypes(), v.getNumberOfChangetracks() + 1, v.getIsBleached());
...
void changeTypeVesicle(Vesicle v, VesicleType vesicleType)
{
    Vesicle newVesicle = Vesicle(v.getVesicleId(), v.getMicrotubuleId(), v.getSiteId(), vesicleType, v.getNumberOfChangetypes() + 1,
                                 v.getNumberOfChangetracks(), v.getIsBleached());
    updateVesicle(v, newVesicle);
    NUMBER_OF_CHANGETYPES++;
}

/*
 * Bleach a region of the axon. This is accomplished by setting the isBleached instance variable of vesicles to true
 */
void bleachSites(int startSite, int endSite)
{
    for (set<Vesicle>::iterator it = VESICLES.begin(); it != VESICLES.end(); it++)
    {
        Vesicle v = (*it);
        if ((v.getSiteId() >= startSite) && (v.getSiteId() <= endSite))
        {
            Vesicle newVesicle = Vesicle(v.getVesicleId(), v.getMicrotubuleId(), v.getSiteId(), v.getType(), v.getNumberOfChangetypes(), v.getNumberOfChangetracks(), true);
            VESICLES.erase(v);
            VESICLES.insert(newVesicle);
        }
    }
}
...
void rouletteWheelUpdate(double r)
{
    ...
    {
        Vesicle v = (*it);
        Vesicle newVesicle = Vesicle(0, 0, 0, SMOOTH_ANTEROGRADE, 0, 0, false);
        ...
        if (hoppingPropensity > 0)
        {
            ...
            if (pastVesiclesPropensities >= r)
            {
                ...
                newVesicle = Vesicle(newVesicleId, v.getMicrotubuleId(), (v.getSiteId() + NUMBER_OF_SITES + (1 * v.getDirection())) % NUMBER_OF_SITES,
                                     v.getType(), v.getNumberOfChangetypes(), v.getNumberOfChangetracks(), v.getIsBleached());
                updateVesicle(v, newVesicle);
                NUMBER_OF_HOPS++;
                return;
            }
...
/*
 * Saves the contents of the axon to the file state.csv
 */
void saveState(string filename)
{
	remove(filename.c_str());
	ofstream stateWriter;
	stateWriter.open(filename.c_str(), ios::out | ios::app);
	for (set<Vesicle>::iterator it = VESICLES.begin(); it != VESICLES.end(); it++)
	{
		ostringstream tmp;
		Vesicle v = (*it);
		tmp << "The vesicle id is : " << v.getVesicleId() << endl;
		tmp << "The vesicle is on microtubule : " << v.getMicrotubuleId() << endl;
		tmp << "The vesicle is at site : " << v.getSiteId() << endl;
		tmp << "The type of the vesicle is : " << v.getType() << endl;
		stateWriter << tmp.str() << endl;
	}
	stateWriter.close();
}
...
void writeStates()
{
	...
	{
	long stateSpacing = NUMBER_OF_UPDATES_TO_SETTLE / 10;
	if(i == 0) 
	{
	long stateSpacing = NUMBER_OF_UPDATES_TO_SETTLE;
	}
		for (int j = 0; j < stateSpacing; j++)
		{
			performUpdate();
		}
		string str = "state";
...
/*
 * Loads the vesicles from state.csv to the axon
 */
void loadState(string filename)
{
    ...
    while (getline(myfile, line))
    {
        ...
        long vesicleId = atol(splitString.at(1).c_str());
        //cout << "The vesicle Id is " << vesicleId << '\n';

        getline(myfile, line);
        splitString = split(line, ':');
        int microtubuleId = atoi(splitString.at(1).c_str());
        //cout << "The microtuble Id is " << microtubuleId << '\n';

        getline(myfile, line);
        splitString = split(line, ':');
        int siteId = atoi(splitString.at(1).c_str());
        //cout << "The site Id is " << siteId << '\n';

        getline(myfile, line);
        splitString = split(line, ':');
        int vesicleType = atoi(splitString.at(1).c_str());
        //cout << "The type of vesicle is " << vesicleType << '\n';

        getline(myfile, line);
        Vesicle newVesicle = Vesicle(vesicleId, microtubuleId, siteId, static_cast<VesicleType> (vesicleType), 0, 0, false);
        VESICLES.insert(newVesicle);
    }
    myfile.close();
}
...
/*
 * Combines string1, int, string2 to string1intstring2
 */
string makeString(string s1, int i, string s2)
{
    s1.append(getStringOf(i));
    s1.append(s2);
    return (s1);
}

vector<int> flattenMicrotubules()
{
    vector<int> vesiclesAtSite;
    vesiclesAtSite.assign(NUMBER_OF_SITES, 0);
    for (set<Vesicle>::iterator it = VESICLES.begin(); it != VESICLES.end(); it++)
    {
        Vesicle v = (*it);
        if (v.getType() != STANDSTILL && v.getIsBleached() == false)
        {
            vesiclesAtSite.at(v.getSiteId()) += 1;
        }
    }
    return vesiclesAtSite;
}

void writeSnapshotToFile(int i)
{
    ofstream densityProfileWriter;
    densityProfileWriter.open(makeString("densityprofile", i, ".csv").c_str(), ios::out | ios::app);
    vector<int> axon = flattenMicrotubules();
    ostringstream tmp;
    for (unsigned int j = 0; j < axon.size(); j++)
    {
        tmp << axon.at(j) << ",";
    }
    densityProfileWriter << tmp.str() << endl;
    densityProfileWriter.close();
}

void axotomize(int sId)
{
	for (int mId = 0; mId < NUMBER_OF_MICROTUBULES; mId++)
	{
		for (int s = sId + 1; s < (sId + AXOTOMY_WIDTH); s++)
		{
			Location location;
			location.microtubuleId = mId;
			location.siteId = s;
			if (isSiteOccupied(location))
			{
				Vesicle vesicle = Vesicle(0, 0, 0, STANDSTILL, 0, 0, false);
				vesicle.setMicrotubuleId(location.microtubuleId);
				vesicle.setSiteId(location.siteId);
				VESICLES.erase(vesicle);
			}
		}
	}
	...
	for (int mId = 0; mId < NUMBER_OF_MICROTUBULES; mId++)
	{
		for (int s = sId + NUMBER_OF_EXCLUDED_NEIGHBOURS + 1; s < (sId + AXOTOMY_WIDTH - NUMBER_OF_EXCLUDED_NEIGHBOURS); s += NUMBER_OF_EXCLUDED_NEIGHBOURS + 1)
		{
			Vesicle newVesicle = Vesicle(generateVesicleId(), mId, s, STANDSTILL, 0, 0, false);
			VESICLES.insert(newVesicle);
		}
	}
}

void generateDensitySequence()
{
    for (int i = 0; i < 200; i++)
    {
    	loadState(makeString("state", i, ".csv"));
        int snapshotNumber = 0;
        double elapsedTime = 0;
        double lastSnapshotTime = 0;
        while (snapshotNumber < NUMBER_OF_DENSITY_SNAPSHOTS) 
        {
        	elapsedTime += performUpdate();
        	if (elapsedTime - lastSnapshotTime > DENSITY_SNAPSHOT_INTERVAL)
            	{
        		writeSnapshotToFile(snapshotNumber);
                	lastSnapshotTime = elapsedTime;
                	snapshotNumber++;
       		}
       		if(snapshotNumber == (NUMBER_OF_DENSITY_SNAPSHOTS / 5)) 
       		{
       			//bleachSites(4000, 5000);
       			axotomize(3950);
       		}
        }
        VESICLES.clear();
